    ...
    <div class="row">
      <div class="col-12">
        <h2 class="page-header"> 
          <center>LAPORAN PENGGAJIAN KARYAWAN</center>
          ...
          <tbody id="hasil_pencarian">
            ...
            <?php 
              ...
              if (isset($_POST['print_tanggal'])) {
                $dari_tanggal = $_POST['dari_tanggal'];
                $sampai_tanggal = $_POST['sampai_tanggal'];
                $query = "SELECT * FROM penggajian WHERE tanggal BETWEEN '$dari_tanggal' AND '$sampai_tanggal'";
              ...
              if (isset($_POST['print_tahun'])) {
                ...
                $tahun = $_POST['tahun'];
                if ($bulan == '') {
                  $query = "SELECT * FROM penggajian WHERE tahun = '$tahun'";
                  $hasil = mysqli_query($conn, $query);
                } else {
                  $query = "SELECT * FROM penggajian WHERE bulan = '$bulan' AND tahun = '$tahun'";
                  $hasil = mysqli_query($conn, $query);
                }
              }
        ...
        <div class="table-responsive">
          <?php 
              ...
              if (isset($_POST['print_tanggal'])) {
                $dari_tanggal = $_POST['dari_tanggal'];
                $sampai_tanggal = $_POST['sampai_tanggal'];
                $query = "SELECT *, count(nik) AS jumlah_nik, sum(total_gaji) AS jumlah_total_gaji FROM penggajian WHERE tanggal BETWEEN '$dari_tanggal' AND '$sampai_tanggal'";
                $hasil = mysqli_query($conn, $query);
              }
              ...
              if (isset($_POST['print_tahun'])) {
                ...
                $tahun = $_POST['tahun'];
                if ($bulan == '') {
                  $query = "SELECT *, count(nik) AS jumlah_nik, sum(total_gaji) AS jumlah_total_gaji FROM penggajian WHERE tahun = '$tahun'";
                  $hasil = mysqli_query($conn, $query);
                } else {
                  $query = "SELECT *, count(nik) AS jumlah_nik, sum(total_gaji) AS jumlah_total_gaji FROM penggajian WHERE bulan = '$bulan' AND tahun = '$tahun'";
                  $hasil = mysqli_query($conn, $query);
                }
              }
              ...
              while($row = mysqli_fetch_assoc($hasil) ){
                echo '<table class="table">
                        ...
                        <tr>
                          <th>Total Gaji</th>
                          <td>'.rupiah($row['jumlah_total_gaji']).'</td>
                        </tr>
                      </table>';
              }
            ?>
